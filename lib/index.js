...
const seoDetect = (() => {
  ...
  const inputHtmlparser = new htmlparser.Parser({
    ...
    onend: () => {
      console.log(' * Info: input file has been processed, parsing rules file');
    },
  });
  ...
  const writeToFile = (outputFilePath) => {
    fs.writeFile(outputFilePath, outputContents, (err) => {
      if (err) console.error('* Error: error writing to file');
      else console.log(' * Info: rules file has been processed, please check output file');
    });
  };
    ...
    ontext: (text) => {
      ...
      switch (rule[0]) {
        ...
        case 'limit':
          ...
          handleLimitRule(currentRule);
          break;

        default:
          console.error(' * Error: invalid rule encountered, skipping rule');
      }
    },

    onend: () => {
      console.log(' * Info: rules file has been processed, please check output file.');
    },
  });
  ...
  return {
    getResults: (args) => {
      ...
      if (!args.inputFilePath) console.warn(` * Warning: input file path not specified, using default file at ${inputFilePath}\n`);
      if (!args.ruleFilePath) console.warn(` * Warning: rule file path not specified, using default file at ${ruleFilePath}`);
      if (!args.outputFilePath) console.warn(` * Warning: Output file path not specified, creating file at ${outputFilePath}`);
